...
* 音频编解码使用纯软方案。
* 视频编解码有三种实现：
  1. FFmpeg硬编解码(FFHardDecoder.cpp、H264FFHardEncoder.cpp)
  ...
  2. FFmpeg纯软编解码(FFSoftDecoder.cpp、H264FFSoftEncoder.cpp)
    * `cmake -DFFMPEG_SOFT=ON ..`
    * 此时代码可以在任何Linux/Windows环境下运行,只需要安装ffmpeg即可
  3. 昇腾DVPP V2版本编解码(DVPPDecoder.cpp、H264DVPPEncoder.cpp、DVPP_utils)
    * `cmake -DDVPP_MPI=ON ..`
    * 默认使用第0号NPU(MiedaWrapper.h)
  4. NVIDIA x86编解码(NVIDIADecoder.cpp、H264NVIDIAEncoder.Nvcodec_utils)
