...
* Video encoding and decoding have three implementations:
  1. FFmpeg hardware codec (FFHardDecoder.cpp, H264FFHardEncoder.cpp)
  ...
  2. FFmpeg pure software codec (FFSoftDecoder.cpp, H264FFSoftEncoder.cpp)
    * `cmake -DFFMPEG_SOFT=ON ..`
    * This code can run on any Linux/Windows environment, only requires FFmpeg installation.
  3. Ascend DVPP V2 codec (DVPPDecoder.cpp、H264DVPPEncoder.cpp、DVPP_utils)
    * `cmake -DDVPP_MPI=ON ..`
    * default uses NPU 0 (MiedaWrapper.h) 
  4. NVIDIA x86 codec(NVIDIADecoder.cpp、H264NVIDIAEncoder.Nvcodec_utils)
