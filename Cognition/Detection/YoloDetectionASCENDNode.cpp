    ...
    while (!abort_) {
        ...
        for(int i = 0; i < packets.size(); i++){
            ...
            if(!packet->use_ptr){
                ...
                CHECK_ACL(aclrtSynchronizeStream(stream_));
                device_image_ptr = packet->context->img_buffer;
            }
            else{
                device_image_ptr = packet->device_image_ptr;
            }
            LetterBox(channel_id_letterbox_, device_image_ptr, img_size, packet->context->width, packet->context->height, (uint8_t*)input_addr_img_ + buffer_idx, model_img_size, input_w_, input_h_);
